...
symbols = symbols = [symbol for symbol in markets if '/USDT' in symbol] # Take only the first 5 symbols
...
for symbol in symbols:
    try:
        ...
        if not ohlcv:  # Check if the data is empty
        ...
        if len(growth_segment) < 2:  # Ensure there's enough data to fit
            print(f"Not enough data to fit growth for {symbol}. Skipping.")
            continue
...
csv_filename = f'crypto_analysis_report_{today_date}.csv'  # Include the date in the filename
